...
rclcpp::Time
TelemetryModule::get_measurement_time(const T_DjiDataTimestamp *timestamp)
{
  ...
  uint64_t fc64_us = clock_sync_.extendTimestamp(timestamp->microsecond);

  clock_sync_.update(fc64_us, ros_now);

  return clock_sync_.toRosTime(fc64_us);
}
                                        ...
                                        const T_DjiDataTimestamp *timestamp)
{
  ...
  if (params_.sim)
  {
    ...
    if (global_gimbal_ptr_->gimbal_mode_is_follow())
    {
      ...
      // std::cout << "setting gimbal done" << std::endl;
    }
                                  ...
                                  const T_DjiDataTimestamp *timestamp)
{
  ...
  // this is done on purpose - battery callback might have a bug and not update
  // the timestamp
...
{  // NOLINT(readability/fn_size)
  ...
  if (params_.control_information_frequency > 0)
  {
    ...
    // be careful - this seems to work only for 1Hz
    return_code = DjiFcSubscription_SubscribeTopic(
...
TelemetryModule::publish_static_transforms()
{
  ...
  if (publish_camera_transforms_)
  {
        ...
        camera_type_ == DJI_CAMERA_TYPE_P1)
    {
      ...
      tf_gimbal.transform.translation.x = 0.01;
...
void
TelemetryModule::publish_dynamic_gimbal_transforms(
    const T_DjiDataTimestamp *timestamp)
{
  if (aircraft_base_.aircraftType == DJI_AIRCRAFT_TYPE_M300_RTK ||
      aircraft_base_.aircraftType == DJI_AIRCRAFT_TYPE_M350_RTK)
